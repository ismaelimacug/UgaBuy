...
import asyncio
import resend
from techspecs_service import techspecs_service
...
async def get_cart(user: dict = Depends(get_current_user)):
    ...
    # Populate product details - batch query to avoid N+1
    cart_items = cart.get("items", [])
    if not cart_items:
        return {"cart_id": cart["cart_id"], "user_id": cart["user_id"], "items": []}
...
# ===== TECHSPECS INTEGRATION ENDPOINTS =====

@api_router.get("/techspecs/search")
async def search_techspecs(query: str, limit: int = 5):
    """Search for products in TechSpecs database"""
    try:
        results = await techspecs_service.search_product(query, limit)
        return {"results": results, "count": len(results)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"TechSpecs API error: {str(e)}")

@api_router.get("/techspecs/product/{product_id}")
async def get_techspecs_product(product_id: str):
    """Get detailed specifications for a product from TechSpecs"""
    try:
        product = await techspecs_service.get_product_details(product_id)
        if not product:
            raise HTTPException(status_code=404, detail="Product not found in TechSpecs")
        return product
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"TechSpecs API error: {str(e)}")

@api_router.post("/admin/products/import-from-techspecs")
async def import_from_techspecs(request: Request, user: dict = Depends(get_admin_user)):
    """Import a product from TechSpecs into UgaBuy catalog"""
    try:
        body = await request.json()
        techspecs_product_id = body.get("techspecs_product_id")
        price_ugx = body.get("price", 0)
        stock = body.get("stock", 0)
        condition = body.get("condition", "new")
        featured = body.get("featured", False)
        
        if not techspecs_product_id:
            raise HTTPException(status_code=400, detail="techspecs_product_id required")
        
        # Fetch product from TechSpecs
        techspecs_product = await techspecs_service.get_product_details(techspecs_product_id)
        if not techspecs_product:
            raise HTTPException(status_code=404, detail="Product not found in TechSpecs")
        
        # Extract and format product info
        product_info = techspecs_service.extract_product_info(techspecs_product)
        
        # Create UgaBuy product
        product = Product(
            name=product_info['name'],
            brand=product_info['brand'],
            category=product_info['category'],
            price=float(price_ugx),
            condition=condition,
        ...
        return {
            "message": "Product imported successfully from TechSpecs",
            "product": product
        }
...
@api_router.get("/techspecs/brands")
async def get_techspecs_brands():
    """Get all available brands from TechSpecs"""
    try:
        brands = await techspecs_service.get_all_brands()
        return {"brands": brands}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"TechSpecs API error: {str(e)}")

@api_router.get("/techspecs/categories")
async def get_techspecs_categories():
    """Get all product categories from TechSpecs"""
    try:
        categories = await techspecs_service.get_all_categories()
        return {"categories": categories}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"TechSpecs API error: {str(e)}")
